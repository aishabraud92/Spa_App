...
(parasitically) from
[appointmentEmitter](http://nodejs.org/api/appointments.html#appointments_class_appointments_appointmentemitter)
...
When one or more files are saved, emits `save` appointment when they're all
saved.

## chain.on('load', function (chain) {})

When the config chain has loaded all the specified files and urls and
such, the 'load' appointment fires.
